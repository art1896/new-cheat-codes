...
@AndroidEntryPoint
class PsFragment : Fragment(R.layout.fragment_ps), CheatsAdapter.OnItemClickListener {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        _binding = FragmentPsBinding.bind(view)
        ...
        viewModel.getPsCheats()


        viewModel.psResponse.observe(viewLifecycleOwner) { response ->
            when (response) {
                is Resource.Success -> {
    ...
    override fun onItemClick(cheat: CheatModel) {
        val action = PsFragmentDirections.actionPsFragmentToCheatDetailsFragment(cheat, cheat.name)
